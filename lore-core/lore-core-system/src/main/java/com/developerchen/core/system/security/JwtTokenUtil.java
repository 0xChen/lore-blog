...
public final class JwtTokenUtil {
    ...
    public static String SECRET_KEY_STR;
    ...
    public static void initSecretKey() {
        ...
        try {
            if (StringUtils.isBlank(SECRET_KEY_PATH)) {
                SECRET_KEY_PATH = SystemConfig.HOME_PATH + File.separator + "jwtSecretKey";
            }
            ...
            tempSecretKey = Keys.hmacShaKeyFor(SECRET_KEY_STR.getBytes(StandardCharsets.UTF_8));
            ;
            logger.warn("使用配置文件中的密钥, {}", e.toString());
        }
    ...
    public static String generateToken(ApiUser apiUser) {
        ...
        claims.put("nickname", apiUser.getNickname());
        claims.put("status", apiUser.getStatus());
        claims.put("email", apiUser.getEmail());
        claims.put("authorities", roles);
        return generateToken(claims, apiUser.getUsername());
    }
    ...
    public static Boolean validateToken(String token, UserDetails userDetails) {
        LocalDateTime updateTime = ((ApiUser) userDetails).getUpdateTime();
        ZonedDateTime zonedUpdateTime = updateTime.atZone(ZoneId.systemDefault());
        long updateTimestamp = zonedUpdateTime.toEpochSecond();

        long tokenIssuedAt = getIssuedAtDateFromToken(token).getTime() / 1000;
        return updateTimestamp <= tokenIssuedAt;
    }
    ...
    public static ApiUser parseTokenToJwtUser(String token) {
