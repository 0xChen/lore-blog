package com.developerchen.blog.module.site.web.admin;
...
import com.developerchen.core.common.exception.RestException;
...
public class SiteAdminController extends BaseController {
    ...
    public R<List<ThemeDTO>> getThemeList() {
     ...
     * 获取主题的参数设置
     */
    @GetMapping("/themes/{themeName}/setting/list")
    public R<List<Option>> getThemeSetting(@PathVariable("themeName") String themeName)
        ...
        String optionJson = SystemConfig.getOption(BlogConst.OPTION_THEME_OPTION_PREFIX + themeName);
        if (optionJson != null) {
     ...
     */
    @GetMapping("/themes/active/detail")
     ...
     * 激活指定主题
     */
    @PostMapping("/themes/{themeName}/active")
     ...
     */
    @PostMapping(path = "/themes/{themeName}/setting/save", consumes = MediaType.APPLICATION_JSON_VALUE)
     ...
     * 站点参数设置
     */
    @PostMapping(path = "/site/setting/save", consumes = MediaType.APPLICATION_JSON_VALUE)
