package com.developerchen.blog.module.link.web.admin;
...
public class LinkAdminController extends BaseController {
    ...
    public R<Link> upsert(@Validated @RequestBody Link link, BindingResult result) {
        if (result.hasErrors()) {
            return R.fail("保存失败！");
        }
    ...
    public R<Link> get(@PathVariable long linkId) {
     ...
     */
    @PostMapping("/{linkIds}/delete-batch")
